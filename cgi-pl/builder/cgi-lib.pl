...
$cgi_lib'maxdata    = 13107200000000;    # maximum bytes to accept via POST - 2^17
...
sub ReadParse {
  ...
  if ($len > $cgi_lib'maxdata) { #'
      &CgiDie("cgi-lib.pl: Request to receive too much data: $len bytes and $cgi_lib'maxdata\n");
  }
  ...
  if (!defined $meth || $meth eq '' || $meth eq 'GET' || 
    ...
    if (!defined $meth || $meth eq '') {
	  ...
	  {$errflag="Short Read2: wanted $len, got $got\n";};
    } else {
...
$errflag = !(eval <<'END_MULTIPART');
   ...
   PART: # find each part of the multi-part while reading data
    while (1) {
      ...
      die "Short Read3: wanted $amt, got $got\n" if $errflag;
      $left -= $amt;
	...
	die "Short Read4: wanted $amt, got $got\n" if $errflag;
        $left -= $amt;
      }
      ...
      $amt = ($left > $bufsize+$maxbound-length($buf) 
	      ? $bufsize+$maxbound-length($buf) : $left);
      $errflag = (($got = read(STDIN, $buf, $amt, length($buf))) != $amt);
      die "Short Read5: wanted $amt, got $got\n" if $errflag;
      $left -= $amt;
     ...
     HEAD:
      while (($lpos = index($buf, "\r\n\r\n")) == -1) { 
        ...
        $amt = ($left > $bufsize ? $bufsize : $left); #$maxbound==length($buf);
        $errflag = (($got = read(STDIN, $buf, $amt, $maxbound)) != $amt);  
        die "Short Read6: wanted $amt, got $got\n" if $errflag;
        $left -= $amt;
      }
