...
print "<HTML>\n";
print "<HEAD><TITLE>$sitetitle Search Results</TITLE></head>\n";
...
$field1=~s/\"//g;
$cntResults++;
$display=$cntResults+$start;
print "\n<a name=\"$cntResults\">\n";
print "<li><font size=2><b><font color='black'>$display.&nbsp;<a href=\"$field4\" target=\"_top\">$field1</a></font></b></font><BR>\n";
...
sub noresults{
 if ($noresults) {
 open (NORESULTS,"<$noresults");
    @noresults = <NORESULTS>;
    close (NORESULTS);
    foreach $line (@noresults) {
           print "$line";
                }
        }
}
