...
# txt2pdf -- Text to PDF
#
# by SANFACE Software <sanface@sanface.com> 19th June 2005
#
# txt2pdf 8.x is shareware.
...
# $120 (US) for 1 CPU computer 
# $140 (US) for 2 or 3 CPU computers 
# $165 (US) for 4 to 7 CPU computers 
# $195 (US) for 8 to 15 CPU computers 
# $230 (US) for 16 to more CPU computers 
#
# You can upgrade txt2pdf 5.x, 6.x, 7.x to the new version with the discount of
...
my $help=0; my $default=0; my $verbose=0; my $Version; my $border=0; my $paper="";
...
my $Unregistered="Generated by unregistered $producer1 v.$version \251 $companyname 2005\nAvailable at $txt2pdfHome \n";
...
From \$120 for the Enterprise Edition.
...
if ($option{'transition'} =~ /replace/) {$transition=""}
  ...
  if (($option{'pagelayout'} !~ /OneColumn/) && ($option{'pagelayout'} !~ /TwoColumnRight/) && ($option{'pagelayout'} !~ /TwoColumnLeft/) && ($option{'pagelayout'} ne "")) {
...
# ISOLatin1Encoding
  ...
  if($option{'japanese'}) {
    $location[++$obj]=$fpos;
      ...
      $buf=sprintf "  /DescendantFonts [ %d 0 R ]\n",$obj+1;
      &pos($buf);
      &pos("  >>\n");
    &pos("endobj\n");
    $location[++$obj]=$fpos;
      ...
      $buf=sprintf "  /FontDescriptor %d 0 R\n",$obj+1;
      &pos($buf);
      &pos("  /CIDSystemInfo\n    <<\n");
      &pos("    /Registry (Adobe)\n");
      &pos("    /Ordering (Japan1)\n");
    ...
    $location[++$obj]=$fpos;
      ...
      &pos("  /StemV 114\n");
      &pos("  /XHeight 553\n");
      &pos("  /Style\n    <<\n     /Panose <0801020b0600000000000000>\n    >>\n");
      &pos("  >>\n");
    &pos("endobj\n");
    $location[++$obj]=$fpos;
...
#  &pos("/OpenAction << /S /GoTo /Dest [ 1 /XYZ null null 1 ] >>\n");
  ...
  if ($option{'viewerpreferences'}) {&pos("  /ViewerPreferences\n    <<\n    $option{'viewerpreferences'}\n    >>\n")}
  if ($option{'fit'}) {&pos("  /OpenAction\n    <<\n    /S /GoTo\n    /D [ 0 /Fit ]\n    >>\n")}
  if ($option{'zoom'}) {&pos("  /OpenAction\n    <<\n    /S /GoTo\n    /D [ 0 /XYZ null null $option{'zoom'} ]\n    >>\n")}
  &pos("  >>\n");
  &pos("endobj\n");
...
sub StartPage {
  ...
  $pageObj[++$pageNO]=$obj;
    ...
    &pos("  /Rotate $rotate\n");
    if ($transition eq "Dissolve") {&pos("  /Trans\n    <<\n    /Type /Trans\n    /S /$transition\n    >>\n")}
    if ($transition eq "Box") {&pos("  /Trans\n    <<\n    /Type /Trans\n     /S /$transition\n    /M /$motion\n    >>\n")}
    if ($transition eq "Glitter") {&pos("  /Trans\n    <<\n    /Type /Trans\n    /S /$transition\n    /Di $direction\n    >>\n")}
    if ($transition eq "Wipe") {&pos("  /Trans\n    <<\n    /Type /Trans\n    /S /$transition\n    /Di $direction\n    >>\n")}
    if ($transition eq "Blinds") {&pos("  /Trans\n    <<\n    /Type /Trans\n    /S\n    /$transition /Dm\n    /$dimension\n    >>\n")}
    if ($transition eq "Split") {&pos("  /Trans\n    <<\n    /Type /Trans\n    /S /$transition\n    /Dm /$dimension\n    /M /$motion\n    >>\n")}
...
# LLx LLy URx URy
  if (($fontn eq 1) || ($fontn eq 2) || ($fontn eq 3) || ($fontn eq 4)) {
    if ($option{'optimize'}) {$buf=sprintf "/Rect[%d %d %d %d]",
...
TXT2PDF - Version 8.2 19th June 2005
